...
export default function LandingSponsor(): React.ReactElement {
  return (
    <MaxContainer>
      <ScreenClassRender
            ...
            >
              ...
              <Col xs={11} lg={6} xl={5}>
                <SponsorLabelContainer
                  style={{ padding: ['lg', 'xl'].includes(sClass) ? '40px' : '24px' }}
                >
                  <Label
                    type={['lg', 'xl'].includes(sClass) ? 'P1' : 'MobileP1'}
                    style={{ marginBottom: '8px' }}
                  >
                    늘 그렇듯이 이런 행사에는 산업체, 개인들의 열화와 같은 지지, 그리고 물심양면의
                    후원이 필요합니다.
                  </Label>
                  <Label
                    type={['lg', 'xl'].includes(sClass) ? 'P1' : 'MobileP1'}
                    style={{ marginBottom: '8px' }}
                  >
                    현금 후원도 받지만, 도서관과 행사 참여 어린이들에게 주실 서적, 기념품, 그리고
                    봉사자들을 위 후원품을 보내주시면 저희가 전달해드립니다. 아주 작은 후원이라도
                    감사히 생각하며 개인, 회사, 기관의 소중한 후원이 우리의 미래를 밝게 할 것입니다.
                  </Label>
                  <Label type={['lg', 'xl'].includes(sClass) ? 'P1' : 'MobileP1'}>
                    또한 후원사들의 명단 & 로고를 널리 알려 이 소중한 행사에 같이 해주심을
                    기억하겠습니다.
                  </Label>
                </SponsorLabelContainer>
                <Visible xl lg>
